...
func Database() *sql.DB {
	...
	if !exist {
		logger.Log(errors.New("DB_USER not found"))
	...
	if !exist {
		logger.Log(errors.New("DB_PASS not found"))
	...
	if !exist {
		logger.Log(errors.New("DB_HOST not found"))
		log.Fatal("DB_HOST not set in .env")
	}

	connectionStr := fmt.Sprintf("%s:%s@(%s:3306)/?charset=utf8&parseTime=True", user, pass, host)
